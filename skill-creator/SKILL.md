---
name: skill-creator
description: Create or update effective Codex skills through a pilot-driven, step-by-step workflow with concrete examples, real dry runs, user correction, validation, and reusable resources. Use when users want to extend Codex with specialized knowledge, workflows, or tool integrations.
---
...
For a new skill, ask where it should ultimately be installed. If the user does not specify a location, use `$CODEX_HOME/skills`, falling back to `~/.codex/skills` when `CODEX_HOME` is unset.
